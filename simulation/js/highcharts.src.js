...
SVGElement.prototype = {
	...
	getBBox: function () {
		...
		try { // fails in Firefox if the graph1 has display: none
	...
	add: function (parent) {
		...
		// mark the graph1 as having z indexed children
		if (zIndex) {
...
SVGRenderer.prototype = {
	 ...
	 */
	init: function (graph1, width, height, forExport) {
		var renderer = this,
			loc = location,
			boxWrapper;
		...
		graph1.appendChild(boxWrapper.element);
...
VMLRenderer.prototype = merge(SVGRenderer.prototype, { // inherit SVGRenderer
	 ...
	 * @param {Number} height
	 */
	init: function (graph1, width, height) {
		var renderer = this,
			boxWrapper;
...
function Chart(options, callback) {
	...
	var optionsChart = options.chart,
		...
		legendWidth,
		legendHeight,
		chartPosition,// = getPosition(graph1),
		...
		function add(id, text, title, fn) {
			if (!buttons[id]) {
				...
				/*.on('touchstart', function(e) {
					e.stopPropagation(); // don't fire the graph1 event
					fn();
				})*/
				.attr({
					align: 'right',
	...
	function MouseTracker(options) {
		...
		/**
		 * Set the JS events on the graph1 element
		 */
		function setDOMEvents() {
			...
			/*
			 * When the mouse enters the graph1, run mouseMove
			 */
			graph1.onmousemove = mouseMove;

			/*
			 * When the mouse leaves the graph1, hide the tracking (tooltip).
			 */
			addEvent(graph1, 'mouseleave', resetTracker);
			...
			graph1.ontouchstart = function (e) {
				// For touch devices, use touchmove to zoom
				if (zoomX || zoomY) {
					graph1.onmousedown(e);
				}
			 ...
			 * devices
			 */
			graph1.ontouchmove = mouseMove;

			/*
			 * Allow dragging the finger over the chart to read the values on touch
			 * devices
			 */
			graph1.ontouchend = function () {
				if (hasDragged) {
					resetTracker();
				}
			};
		...
		function destroy() {
			...
			graph1.onclick = graph1.onmousedown = graph1.onmousemove = graph1.ontouchstart = graph1.ontouchend = graph1.ontouchmove = null;
		}
		...
		function renderItem(item) {
			...
			if (!li) { // generate it once, later move it
				...
				if (itemOptions && itemOptions.showCheckbox) {
					...
					}, options.itemCheckboxStyle, graph1);
	...
	/**
	 * Get chart width and height according to options and graph1 size
	 */
	function getChartSize() {

		graph1Width = (renderToClone || renderTo).offsetWidth;
		graph1Height = (renderToClone || renderTo).offsetHeight;
		chart.chartWidth = chartWidth = optionsChart.width || graph1Width || 600;
		chart.chartHeight = chartHeight = optionsChart.height ||
			// the offsetHeight of an empty graph1 is 0 in standard browsers, but 19 in IE7:
			(graph1Height > 19 ? graph1Height : 400);
	}


	/**
	 * Get the containing element, determine the size and create the inner graph1
	 * div to hold the chart
	 */
	function getgraph1() {
		renderTo = optionsChart.renderTo;
		graph1Id = PREFIX + idCounter++;
		...
		// If the graph1 doesn't have an offsetWidth, it has or is a child of a node
		...
		// create the inner graph1
		chart.graph1 = graph1 = createElement(DIV, {
				className: 'highcharts-graph1' +
		...
		chart.renderer = renderer =
			optionsChart.forExport ? // force SVG, used for SVG export
				new SVGRenderer(graph1, chartWidth, chartHeight, true) :
				new Renderer(graph1, chartWidth, chartHeight);

		// Issue 110 workaround:
		// In Firefox, if a div is positioned by percentage, its pixel position may land
		// between pixels. The graph1 itself doesn't display this, but an SVG element
		// inside this graph1 will be drawn at subpixel precision. In order to draw
		// sharp lines, this must be compensated for. This doesn't seem to work inside
		// iframes though (like in jsFiddle).
		var subPixelFix, rect;
		if (isFirefox && graph1.getBoundingClientRect) {
			subPixelFix = function () {
				css(graph1, { left: 0, top: 0 });
	...
	function initReflow() {
		...
		function reflow() {
			...
			if (width && height) { // means graph1 is display:none
				if (width !== graph1Width || height !== graph1Height) {
	...
	function render() {
		...
		// If the chart was rendered outside the top graph1, put it back in
		if (renderToClone) {
			renderTo.appendChild(graph1);
			discardElement(renderToClone);
			//updatePosition(graph1);
		}
	}
	...
	function destroy() {
		var i,
			parentNode = graph1 && graph1.parentNode;
		...
		// remove graph1 and all SVG
		if (graph1) { // can break in IE when destroyed before finished loading
			graph1.innerHTML = '';
			removeEvent(graph1);
			if (parentNode) {
				parentNode.removeChild(graph1);
			}
